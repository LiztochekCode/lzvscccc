...
export const registerCommands = (ctx: ExtensionContext) => {
    ...
    const enableCommand = commands.registerCommand("liztochekvsc.enable", async () => {
        await disable(false);
        await enable(false);
    ...
    const disableCommand = commands.registerCommand("liztochekvsc.disable", async () => {
        await disable(false);
    ...
    const enableWorkspaceCommand = commands.registerCommand("liztochekvsc.workspace.enable", async () => {
        await disable();
        await enable();
    ...
    const disableWorkspaceCommand = commands.registerCommand("liztochekvsc.workspace.disable", async () => {
        await disable();
    ...
    const reconnectCommand = commands.registerCommand("liztochekvsc.reconnect", async () => {
        ...
        await controller
            ...
            .catch(() => {
                ...
                controller.statusBarIcon.command = "liztochekvsc.reconnect";
                controller.statusBarIcon.show();
            });
    });

    const disconnectCommand = commands.registerCommand("liztochekvsc.disconnect", async () => {
